...
#define daylight _daylight
	...
	m_bSnowyItem = m_pKVItem->GetInt("snowyitem", 0) != 0;
...
,   m_mapSnowyItems( DefLessFunc(int) )
,	m_unVersion( 0 )
#if defined(CLIENT_DLL) || defined(GAME_DLL)
...
bool CEconItemSchema::BInitTextBuffer( CUtlBuffer &buffer, CUtlVector<CUtlString> *pVecErrors /* = NULL */ )
{
	...
	if (m_pKVRawDefinition->LoadFromFile(g_pFullFileSystem, "scripts/items/items_snowy.txt", "GAME"))
	{
		return BInitSchema( m_pKVRawDefinition, pVecErrors )
			&& BPostSchemaInit( pVecErrors );
	}
	...
	if ( NULL != pKVItems )
	{
		FOR_EACH_TRUE_SUBKEY( pKVItems, pKVItem )
		{
			...
			else
			{
				...
				if ( pItemDef->IsSnowyItem() )
				{
					m_mapSnowyItems.Insert( nItemIndex, pItemDef );
				}	
